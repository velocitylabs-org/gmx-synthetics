...
```
HOW MODULES COMMUNICATE (In-Memory)
═══════════════════════════════════

1. EVENT WATCHER sees OrderCreated
   │
   └──► queue.add({ type: 'ORDER', priority: 5, data: orderEvent })
        (Just a function call, no network!)

2. POSITION MONITOR detects liquidation risk
   │
   ├──► queue.add({ type: 'LIQUIDATION', priority: 10, data: position })
   │
   └──► notificationService.sendAlert(user, "Position at risk!")
        (Just a function call!)

3. EXECUTOR pulls from queue
   │
   ├──► const job = await queue.getNext()  // Highest priority first
   │
   ├──► const prices = await priceFetcher.getPrices(job.tokens)
   │
   └──► await blockchain.executeOrder(job.key, prices)


NO NETWORK CALLS BETWEEN MODULES - Just function calls and shared memory!
```

### Recommended Tech Stack for Nivo

```
NIVO KEEPER - RECOMMENDED STACK
═══════════════════════════════
...
│   FRAMEWORK:       None needed (or Fastify for health endpoints)            │
│   QUEUE:           BullMQ (Redis-backed) or in-memory for MVP               │
│   BLOCKCHAIN:      ethers.js v6                                             │
│   LOGGING:         Pino (fast, JSON logs)                                   │
│   DEPLOYMENT:      Docker (single container)                                │
│   MONITORING:      PM2 or simple health endpoint                            │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│   ├── modules/
│   │   ├── eventWatcher.ts   # Listens to blockchain events
│   │   ├── priceFetcher.ts   # Fetches from Pyth
│   │   ├── positionMonitor.ts # Checks positions health & end dates
│   │   ├── notifier.ts       # Sends alerts, logs events
│   │   └── executor.ts       # Executes transactions (has private key)
│   ├── queue/
│   │   ├── executionQueue.ts # Priority queue implementation
...
> **📝 NIVO NOTE:** Don't over-engineer. A well-structured TypeScript monolith with 5 internal modules and a shared queue will be easier to build, deploy, debug, and maintain. You can always refactor later if you hit scale problems (you won't for a long time with typical keeper volumes).
