...
## Scope Basis
- Research basis: Gating MarketSwap call-path analysis (A-E) used in SCRUM225/226 governance rationale.
...
| E | Single-token caveat and governance disable posture | `contracts/swap/SwapUtils.sol` comment/validation path for single-token context + same-token market policy checks | `scripts/configs/verifySameTokenInvariants.ts` (same-token market invariants), `test/config/VerifySwapReconfiguration.ts` (swap-type gating) | Partial | Explicit negative-path test for "attempt swap on single-token market and assert expected revert/disable reason" is not yet present; accepted as residual test gap for SCRUM228 scope. |

## Missing / Partial Coverage Handling
- Path E is marked `Partial`.
- Explicit risk acceptance for SCRUM227:
  - Current controls provide policy-level protection through feature-flag gating plus same-token invariant checks.
  - A direct negative-path single-token swap test is deferred to SCRUM228 (test hygiene/redaction ticket) as the designated test-hardening scope.

## Conclusion
- Paths A-D: adequately covered by existing tests and validation scripts.
- Path E: operationally controlled, but test completeness gap remains and is tracked as accepted residual risk pending SCRUM228.
