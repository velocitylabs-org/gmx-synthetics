...
**Flow:** Keeper calls `AdlHandler.executeAdl(account, market, collateralToken, isLong, oracleParams)` to run Auto-Deleveraging (ADL) and reduce exposure when needed.
...
**Flow:** Keeper calls `LiquidationHandler.executeLiquidation(account, market, collateralToken, isLong, oracleParams)` to liquidate underwater positions, or `executeCloseExpired(key)` which uses the same handler with an expiry key.
...
Keeper
  ...
  → OrderHandler path (liquidation is executed as a decrease order)
```

**Also:** `LiquidationHandler.executeCloseExpired(key)` — same handler, used for closing expired positions (internally calls executeLiquidation with expiry semantics).
