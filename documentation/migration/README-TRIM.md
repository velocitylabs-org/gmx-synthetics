...
**This document is trimmed to Perpetuals and Liquidity Provisioning only.** Spot trading (swaps) and unrelated order-type details are omitted.
...
Markets support perp (leverage) trading; they are created by specifying a long collateral token, short collateral token and index token.
...
The contracts support the following main features (as covered in this doc):
...
- Order keepers: checks for deposit / withdraw liquidity requests, order requests (e.g. perp increase/decrease), bundles the signed oracle prices with the requests and executes them
...
For deposits and withdrawals, imbalance is calculated as the difference in the worth of the long tokens and short tokens in the pool.
...
`price impact exponents` and `price impact factors` are configured per market and can differ for pool (deposit/withdrawal) and position actions.
...
Price impact is also tracked using a virtual inventory value for positions (and for pool balance across similar markets).
...
There are configurable position fees per market (for perp increase/decrease). Execution fees are also estimated and accounted for on creation of deposit, withdrawal, and order requests so that keepers can execute transactions at a close to net zero cost.
...
- Price impact can be reduced by using positions and trading across markets, chains, forks, other protocols, this is partially mitigated with virtual inventory tracking
