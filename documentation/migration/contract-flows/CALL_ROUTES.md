...
**Note:** `msg.value` is shared across all delegatecalls in a single multicall. `sendWnt` uses it for wrapping; subsequent steps receive the same `msg.value`.
...
**Flow:** Keeper calls `DepositHandler.executeDeposit(key, oracleParams)` with signed oracle prices. Handler validates gas, runs `_executeDeposit` via try/call with gas limit, then `ExecuteDepositUtils.executeDeposit`.
...
Keeper
  ...
  → ExecuteDepositUtils.executeDeposit
      → MarketUtils, DepositVault, DataStore, EventEmitter, DepositStoreUtils, ...
```
...
**Flow:** Keeper calls `WithdrawalHandler.executeWithdrawal(key, oracleParams)`.
...
Keeper
  ...
  → GasUtils.estimateExecuteWithdrawalGasLimit, validateExecutionGas, getExecutionGas
...
**Flow:** Keeper calls `OrderHandler.executeOrder(key, oracleParams)`. Handler gets order executor (Increase/Decrease/Swap) and runs `ExecuteOrderUtils.executeOrder`.
...
Keeper
  ...
  → doExecuteOrder
  → getOrderExecutor (IncreaseOrderExecutor / DecreaseOrderExecutor / SwapOrderExecutor)
...
**Flow:** Keeper calls `ShiftHandler.executeShift(key, oracleParams)`.
...
**Flow:** User calls `ExchangeRouter.executeAtomicWithdrawal(params, oracleParams)` — user supplies oracle params (e.g. from a relayer), so create + execute happen in one transaction.
...
EOA
  ...
  → WithdrawalUtils.createWithdrawal (creates)
  → ExecuteWithdrawalUtils.executeWithdrawal (executes same tx)
```
...
- **JitOrderHandler.executeJitOrder** — `onlyOrderKeeper`, `withOraclePrices` (JIT = just-in-time order, create+execute in one)
...
| **GasUtils** | estimateExecuteDepositGasLimit, estimateExecuteWithdrawalGasLimit, estimateExecuteOrderGasLimit, validateExecutionGas, getExecutionGas |
...
```
  ...
  └────┬─────┘ └────┬─────┘ └────┬─────┘     └────┬─────┘                           │
     ...
     │  Keys        │             │  OracleUtils│  AccountUtils    │
     ├──────────────┼──────────────┼──────────────┼──────────────────┤
     │ DepositVault │WithdrawalVault│ OrderVault  │ MultichainVault  │
     │ ShiftVault  │              │             │                  │
     ├──────────────┼──────────────┼──────────────┼──────────────────┤
     │MarketUtils   │PositionUtils │ SwapUtils   │ ReferralUtils   │
     │DepositStore  │OrderStore   │ Withdrawal   │ CallbackUtils   │
     │Utils         │Utils        │ StoreUtils   │ NonceUtils      │
     │ShiftStoreUtils│             │             │ RoleStore       │
     └──────────────┴──────────────┴──────────────┴──────────────────┘
```
