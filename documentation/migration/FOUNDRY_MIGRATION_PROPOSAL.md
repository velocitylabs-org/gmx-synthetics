...
**Operational sequencing (Velocity / `SCRUM126-SCRUM127`):** See **`documentation/migration/MIGRATION_STATUS.md`** for the current rollout: **test updates and Foundry migration first**; **trim `deploy/` scripts after** tests define the required protocol surface. The phase numbers below are the original proposal; map them to that doc’s Phase 0–3 table for day-to-day work.
